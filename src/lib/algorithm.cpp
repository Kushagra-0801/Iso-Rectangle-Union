...
std::tuple<std::vector<Interval>, std::vector<Interval>, std::vector<Coord>, std::vector<Stripe>> stripes(std::vector<Edge> &v, Interval x_ext)
{
    if (v.size() == 1)
    {
        ...
        std::vector<Coord> p{INT32_MIN, v[0].interval().bot, v[0].interval().top, INT32_MAX};
        std::vector<Stripe> S;
    ...
    else
    {
        ...
        Coord xm = (v[median].coord() + std::max_element(v1.begin(), v1.end())->coord()) / 2;
...
std::vector<Interval> partition(std::vector<Coord> &y)
{
    ...
    std::adjacent_difference(y.begin(), y.end(), std::back_inserter(res), [](auto a, auto b) { return Interval(a, b); });
    return res;
}
